...
module.exports = {
    sendMessage: async (data) => {
        ...
        try {
         ...
         VALUES (?, ?, ?, ?, ?, ?)`,
            ...
            if (result)
                return { success: true, message: 'Message Sent Successfully', data: [{ messageID, senderID, receiverID, chatID, message, messageType, isRead: 0, timestamp: moment().format('YYYY-MM-DD HH:mm:ss') }] };
    ...
    markAllPreviousReadByReceiverID: async (data) => {
        ...
        try {
            const result = await sqlQuery(`UPDATE db_chat_messages SET isRead = 1 WHERE receiverID = ? AND chatID = ? AND isRead = 0`, [receiverID, chatID]);
            if (result)
                return { success: true, message: 'Messages marked as read successfully!', data: result };
            return { success: false, message: "something Went wrong While SQL Query", data: [] };
        } catch (error) {
            console.error('Error sending msg:', error);
            return { success: false, message: error.message };
    ...
    markMessageAsSeenByID: async (data) => {
        ...
        try {
            ...
            if (result)
                return { success: true, message: 'Message marked as seen successfully', data: result };
            return { success: false, message: "something Went wrong While SQL Query", data: [] };
        } catch (error) {
            console.error('Error sending msg:', error);
            return { success: false, message: error.message };
    ...
    getChatParticipants: async (data) => {
        ...
        try {
            ...
            if (result)
                return { success: true, message: 'Participants found successfully!', data: result };
            return { success: false, message: "something Went wrong While SQL Query", data: [] };
        } catch (error) {
            console.error('Error sending msg:', error);
            return { success: false, message: error.message };
        ...
        try {
            var result = await sqlQuery(
                ...
                WHERE
                    ...
                    OR (
                        -- Show group/broadcast chats where your ID is in db_chat_members
            ...
            totalCount = await sqlQuery(`SELECT count(1) as count FROM db_chats WHERE createdBy = ? OR chatWith = ?`, [userID, userID]);
    ...
    createNewChat: async (data) => {
        ...
        try {
            ...
            return { success: false, message: "Something Went wrong While SQL Query", data: [chatID, !chatName ? 'NULL' : chatName, chatType, createdBy] };
        } catch (error) {
            console.error('Error sending msg:', error);
            return { success: false, message: error.message };
        }
    ...
    addUserToGroup: async (data) => {
        ...
        try {
            const result = await sqlQuery(
                `INSERT INTO db_chat_members (chatID, userID) VALUES (?, ?)`,
                [chatID, userID]
            );
            if (result)
                return { success: true, message: 'Chat Created Successfully', data: [{ chatID, userID }] };
    ...
    markMessageAsSeen: async (data) => {
        ...
        try {
            ...
            if (result)
                return { success: true, message: 'Message marked as seen successfully', data: [{ messageID }] };
