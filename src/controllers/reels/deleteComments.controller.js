...
module.exports = () => {
  return async (req, res) => {
    ...
    try {
      const getComment = await sqlQuery(`
        SELECT reelId FROM db_reel_comments WHERE commentId = '${commentId}'
      `);
      ...
      await sqlQuery(`
        DELETE FROM db_reel_comments WHERE commentId = '${commentId}'
      `);
      ...
      res.status(200).json({
        status: true,
        msg: 'Comment deleted and count updated successfully',
        data: null
      });
    } catch (error) {
